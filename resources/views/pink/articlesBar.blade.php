<div class="widget-first widget recent-posts">
	<h3>Recent Arts{{-- Lang::get('ru.latest_projects') --}}</h3>
	<div class="recent-post group">
	
	@if(!$portfolios->isEmpty())
		@foreach($portfolios as $portfolio)
			<div class="hentry-post group">
				<div class="thumb-img"><img style="width:55px !important" src="{{ asset(config('app.theme')) }}/images/projects/{{ $portfolio->img->mini }}" alt="" title="{{ $portfolio->title }}" /></div>
				<div class="text">
					...
					<a class="read-more" href="{{ route('portfolios.show',['alias'=>$portfolio->alias]) }}">&rarr; Read More{{-- Lang::get('ru.read_more') --}}</a>
				</div>
			</div>
		@endforeach
	@endif
...
<h3>Recent Comments{{-- Lang::get('ru.latest_comments') --}}</h3>
